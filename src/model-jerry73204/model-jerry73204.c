...
struct trip trips[MAX_NUM_TRIPS];
int num_trips;
struct trip trip_pointers[MAX_NUM_TRIPS];
...
#pragma omp parallel for
    for (int i = 0; i < num_chunks; i++)
    {
        ...
        while (ptr != end)
        {
            ...
            trip_ptr->trip_id = strtoull(ptr, &str_end, 10);
            ptr = str_end + 3;
...
int main(int argc, char **argv)
{
    ...
    positions = (struct coordinate*) malloc(MAX_NUM_POSITIONS * sizeof(struct coordinate));
